...
namespace CMKZ {
    ...
    public class List<T> : System.Collections.Generic.List<T> {
        ...
        internal List<T> MoveTo(int Index,Func<T, bool> value) {
        ...
        public List<T1> Where<T1>() where T1 : class {
            var A = new List<T1>();
            foreach (var i in this) {
                if (i is T1) {
                    A.Add(i as T1);
        ...
        //except
        public List<T> Except<T2>() { 
            return Except(t => t is T2); 
        }
        public List<T> Except(Func<T, bool> X) {
            var A = new List<T>();
            foreach (var i in this) {
                if (!X(i)) {
                    A.Add(i);
