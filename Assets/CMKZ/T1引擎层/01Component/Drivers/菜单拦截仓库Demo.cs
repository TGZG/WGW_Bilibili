...
namespace CMKZ.仓库Demo {
    ...
    public static class 仓库驱动Demo {
        ...
        public static void Main() {
            ...
            var B = MainPanel.LoadPrefab("仓库").GetComponent<仓库驱动>();
            B.芥子模板.On芥子销毁 += (数据) => {
                Alert("Steam库存减少");
                return true;
            };//在类的实现Remove里书写
            B.芥子模板.On芥子转移 += (数据, 新仓) => {
                //如果从背包转移到争霸区，且目前在争霸区，那么要献祭
                //如果从争霸区转移到安全区，那么禁止
                if (数据.RootParent != 新仓.RootParent) {
                    Print("禁止");
                    return false;
                }
                Print("同意");
        ...
        public static I仓库数据 Load() {
            return 仓库数据唯一 = TryFileRead("仓库存档.json", () => {
                ...
                执行X次(10, i => {
                    var D = C.Create($"仓库{i}");
                    10.TimesToDo(i => D.AddItem(I仓库元素设定.AllSettings.Keys.Choice(), RandomLog(1, 10000_0000)));
                });
                return B;
            });
        }
